...
const addOffer = async (req, res) => {
  ...
  try {
    ...
    if (offerType === "product" && applicableProducts.length === 0) {
      return res
        .status(400)
        .json({ message: "Products must be selected for product offers" });
    }

    if (offerType === "category" && applicableCategories.length === 0) {
      return res
        .status(400)
        .json({ message: "Categories must be selected for category offers" });
    }
    ...
    const existingOffer = await offerSchema.findOne({ title: lowercaseTitle });

    if (existingOffer)
      return res.status(400).json({ message: "this offer already existing" });
    // Create new offer
...
const editOffer = async (req, res) => {
  ...
  try {
    ...
    if (
      offerType === "referral" &&
      referralCode !== existingOffer.referralCode
    ) {
      const duplicateOffer = await offerSchema
    ...
    const exist = await offerSchema.findOne({
      title: lowercaseTitle,
      _id: { $ne: id },
    });
    if (exist)
    ...
    // Update offer
    existingOffer.title = lowercaseTitle;
...
const getOfferById = async (req, res) => {
  ...
  if (!offerId)
    return res.status(400).json({ message: "the offerid is needed here" });
  try {
